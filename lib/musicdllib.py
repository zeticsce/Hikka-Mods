...
logger = logging.getLogger('MusicDLForkLib')


class MusicDLForkLib(loader.Library):
    ...
    def set_module(self, mod: loader.Module) -> None:
        self.mod = mod
        self.ex_lossless = mod.pointer('losslessUnavailable', [])
        logger.info('MusicDLForkLib loaded! version: %s', self.version)
    ...
    async def dl(
        ...
        try:
            ...
            if (
                (self.config["lossless_priority"] and full_name not in self.ex_lossless)
                or not document
            ):
                try:
                    ...
                    async def handler(event: MessageEdited):
                        ...
                        try:
                            if (
                                ...
                                if (
                                    self.mod and getattr(
                                        document := event.message.document, 'id', 0
                                    ) == 5786173421410844775
                                ):
                                    if full_name not in self.ex_lossless:
                                        self.ex_lossless.append(full_name)
